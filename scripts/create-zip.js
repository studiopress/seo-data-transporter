...
function runBuildSteps() {
	...
	try {
		...
		execSync(
            "wp i18n make-pot . languages/seo-data-transporter.pot --exclude=config,node_modules,scripts,vendor --headers='{ \"Report-Msgid-Bugs-To\": \"StudioPress <translations@studiopress.com>\" }' --exclude=bin/ --quiet",
			{ stdio: "inherit" }
		);
...
function createZip() {
	...
	const zipFileName = `seo-data-transporter.${version}.zip`;
	...
	function addDirectoryToZip(directory) {
		...
		for (const file of files) {
			...
			if (stats.isDirectory()) {
				...
				zip.addFile(
					path.join("seo-data-transporter", relativePath),
